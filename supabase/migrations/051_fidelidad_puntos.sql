-- ═══════════════════════════════════════════════════════════════
--  051 — Programa de fidelidad (puntos)
--  Regla: cada L 26 facturados = 1 punto · 1 punto = L 1 en laboratorio
-- ═══════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_ppm_factura  ON paciente_puntos_movimientos(factura_id);
CREATE INDEX IF NOT EXISTS idx_ppm_fecha    ON paciente_puntos_movimientos(created_at DESC);
...
COMMENT ON TABLE paciente_puntos_movimientos IS 'Libro de puntos de fidelidad por paciente';
COMMENT ON COLUMN pacientes.puntos IS 'Saldo actual de puntos (caché; auditar en paciente_puntos_movimientos)';

-- ── Acumular puntos al emitir factura fiscal ───────────────────
...
-- ── Canjear puntos en cobro de laboratorio ─────────────────────
...
BEGIN
  ...
  SELECT COALESCE(puntos, 0) INTO v_saldo_actual
  FROM pacientes
  ...
  IF v_canje <= 0 THEN
    RAISE EXCEPTION 'El paciente no tiene puntos suficientes';
  END IF;
  ...
  INSERT INTO paciente_puntos_movimientos (
    paciente_id, tipo, puntos, saldo_despues,
    caja_movimiento_id, nota, usuario_id
  ) VALUES (
    p_paciente_id,
    'CANJE',
    -v_canje,
    v_nuevo,
    p_movimiento_id,
    COALESCE(p_nota, 'Canje laboratorio — ' || v_canje || ' pt(s)'),
    auth.uid()
  );
...
CREATE POLICY "ppm_select"
  ON paciente_puntos_movimientos FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "ppm_insert"
  ON paciente_puntos_movimientos FOR INSERT TO authenticated
  WITH CHECK (true);
