-- ═══════════════════════════════════════════════════════════════
--  FIX URGENTE — Error al cerrar caja (RLS caja_sesiones)
--  Ejecutar en Supabase → SQL Editor → Run
--  Copia de supabase/migrations/046_caja_cierre_rls.sql
-- ═══════════════════════════════════════════════════════════════
  ...
  SELECT EXISTS (
    ...
    WHERE s.id = p_sesion_id
      ...
      AND (
        ...
        OR (s.cajero_id IS NULL AND fn_caja_puede_ventas())
        OR (fn_caja_es_admin() AND fn_caja_puede_ver_sucursal(s.sucursal_id))
      )
  );
$$;
...
CREATE POLICY "caja_sesiones_update"
  ...
  WITH CHECK (
    (cajero_id = auth.uid() AND estado IN ('ABIERTA', 'CERRADA'))
    OR (fn_caja_es_admin() AND fn_caja_puede_ver_sucursal(sucursal_id) AND estado IN ('ABIERTA', 'CERRADA'))
    OR (fn_caja_puede_ventas() AND cajero_id IS NULL AND estado IN ('ABIERTA', 'CERRADA'))
  );
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true, 'id', p_sesion_id, 'estado', 'CERRADA');
END;
$$;
