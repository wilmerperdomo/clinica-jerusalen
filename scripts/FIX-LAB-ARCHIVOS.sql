...
-- Crea lab_archivos, bucket lab-resultados y columna resultado_externo
...
DECLARE pol RECORD;
BEGIN
  FOR pol IN SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'lab_archivos' LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON lab_archivos', pol.policyname);
  END LOOP;
END $$;
...
SELECT 'lab_archivos' AS tabla, COUNT(*) AS filas FROM lab_archivos;
SELECT id, name, public FROM storage.buckets WHERE id = 'lab-resultados';
