...
import { procesarRecordatoriosAgenda } from '@/lib/agenda-recordatorios-sender'
...
async function procesar(req: Request, permitirSesion = false) {
  ...
  if (!supabase) {
    return NextResponse.json({
      ok: false,
      error: 'Falta SUPABASE_SERVICE_ROLE_KEY para procesar recordatorios automáticos.',
    }, { status: 500 })
  }

  const res = await procesarRecordatoriosAgenda(supabase)
